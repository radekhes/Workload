...
def calc_workload(mod,workload, staff, course, course_staff):
    ...
    try:
        ...
        work2=eval(str(work)+'*(W*0.8 + coord*0.2)');
...
def admin_assign_course_staff():
    ...
    if year:
        ...
        course_staff=CourseStaff(course.id,staff.id,float(workload),coord,year_db.id, 0, staff_name)
        ...
        num_cs=len(cs_set)
        total_work=float(workload)
        for cs in cs_set:
             total_work=total_work+cs.workload

        if total_work>1:
            return staff_home(VIEW_STAFF=staff_name_clean, YEAR=year_db.number, error_msg='Assignment not updated: Error total work must be less than or equal to 1.')
        ...
        if cs_orig:
           if (cs_orig.is_coordinator!=coord and coord==1):
             ...
             #     db.session.add(cs)
              ...
              current_app.logger.error("This is the course name:   " + str(coord) + " " + str(staff.id))
